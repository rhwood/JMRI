...
public class ConnectionsPreferencesPanel extends JTabbedPane implements ManagingPreferencesPanel {
    ...
    private void addConnection(int tabPosition, final JmrixConfigPane configPane) {
        ...
        final JCheckBox disable = new JCheckBox(Bundle.getMessage("ButtonDisableConnection"));
        ...
        if (configPane.getConnectionName() != null) {
            ...
            title = Bundle.getMessage("TabbedLayoutConnection") + (tabPosition + 1);
            if (this.indexOfTab(title) != -1) {
                for (int x = 2; x < 12; x++) {
                    title = Bundle.getMessage("TabbedLayoutConnection")
                            + (tabPosition + 2);
                    if (this.indexOfTab(title) != -1) {
                        break;
                    }
    ...
    private void newConnectionTab() {
        this.addTab(null, addIcon, null, Bundle.getMessage("ToolTipAddNewConnection"));
        ...
        if (i != -1) {
            ...
            if (e != null) {
                int n = JOptionPane.showConfirmDialog(null, MessageFormat.format(
                        Bundle.getMessage("MessageDoDelete"),
                        new Object[]{this.getTitleAt(i)}),
                        Bundle.getMessage("MessageDeleteConnection"),
