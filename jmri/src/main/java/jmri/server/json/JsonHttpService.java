 ...
 * <p>
 * A message ID from a client is a positive integer greater than zero, to be
 * passed back unchanged to the client so the client can track direct responses
 * to requests (this is not needed in the RESTful API, but is available in the
 * RESTful API). The Message ID (or zero if none) is passed into most public
 * methods of JsonHttpService as the {@code id} parameter. When creating an
 * object that is to be embedded in another object as a property, it is
 * permissable to pass the additive inverse of the ID to ensure the ID is not
 * included in the embedded object, but allow any error messages to be thrown
...
public abstract class JsonHttpService {
     ...
     */
    @Nonnull
    public JsonNode doPut(@Nonnull String type, @Nonnull String name, @Nonnull JsonNode data,
            @Nonnull JsonRequest request)
            throws JsonException {
        throw new JsonException(HttpServletResponse.SC_METHOD_NOT_ALLOWED,
                Bundle.getMessage(request.locale, "PutNotAllowed", type), request.id);
    }
     ...
     */
    public void doDelete(@Nonnull String type, @Nonnull String name, @Nonnull JsonNode data,
            @Nonnull JsonRequest request)
            throws JsonException {
        throw new JsonException(HttpServletResponse.SC_METHOD_NOT_ALLOWED,
     ...
     */
    public final boolean acceptForceDeleteToken(@Nonnull String type, @Nonnull String name,
    ...
    public final void throwDeleteConflictException(@Nonnull String type, @Nonnull String name,
        ...
        throw new JsonException(HttpServletResponse.SC_CONFLICT,
                Bundle.getMessage(request.locale, "ErrorDeleteConflict", type, name), data, request.id);
    }
     ...
     * @see #message(ObjectMapper, String, JsonNode, String, int)
     */
    public static final JsonNode message(@Nonnull ObjectMapper mapper, @Nonnull ArrayNode data,
            @CheckForNull String method, int id) {
     ...
     * @see #message(ObjectMapper, ArrayNode, String, int)
     */
    public static final ObjectNode message(@Nonnull ObjectMapper mapper, @Nonnull String type, @Nonnull JsonNode data,
            @CheckForNull String method, int id) {
